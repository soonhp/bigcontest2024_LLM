...
def route_and_intent_analysis(llm, state: GraphState):
   ...
   placeholder.markdown("검색형 질문 / 추천형 질문 분류 중...", unsafe_allow_html=True)
   ...
   if state['query_type'] == 'search':
      placeholder.markdown("분류 결과 : 검색형 질문", unsafe_allow_html=True)
   if state['query_type'] == 'recomm':
      placeholder.markdown("분류 결과 : 추천형 질문", unsafe_allow_html=True)
   return state
